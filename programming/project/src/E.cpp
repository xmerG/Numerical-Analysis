...
//check if 2 splines are equal
bool is_equal(vector<Polynomial> &p1, vector<Polynomial> &p2){
    if(p1.size()!=p2.size()){
...
int main(){
    F1_x f1;
    F1_y f2;
    cubic_bspline_fit fit1(10, 0, 6*pi, f1, f2,boundaryType::natural);
    vector<Polynomial> p1=fit1.getpol();
    fit1.print("E_curve_r2.txt");

    cubic_ppform_fit fit2(10, 0, 6*pi, f1, f2,boundaryType::natural);
    vector<Polynomial> p2=fit2.getpol();
    fit2.print("E_curve_r2.txt");
    bool check1=is_equal(p1,p2);
    bool check2=is_almost_equal(p1,p2);
    cout<<"the consequence of 2 forms of spline is "<<check1<<endl;
